...
const Channel = (props) => {
    ...
    }, [axiosPrivate, channelId, location, navigate]) // channel  - updates when members array changes but causes many requests to be fired
    ...
    }, [refreshChannel, axiosPrivate, channelId])
    ...
    useEffect(() => {
        const updateChannel = async () => {
            try {
                await axiosPrivate.get(`/channels/channel/${channelId}/user/${auth.user}/userId/${auth.id}`);
            } catch (err) {
                console.error(err);
            }
        }
    ...
    }, [auth.id, auth.user, axiosPrivate, channelId])
